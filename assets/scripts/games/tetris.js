(() => {
    ...
    function randPiece() {
        ...
        return { shape: pieces[k].map(r => r.slice()), color: colors[keys.indexOf(k)] };
    }
    function rotate(m) {
        const H = m.length;
        const W = m[0].length;
    ...
    function collide(b, p, ox = 0, oy = 0) {
        ...
        for (let y = 0; y < m.length; y++) for (let x = 0; x < m[y].length; x++) {
            if (!m[y][x]) continue; const bx = p.x + x + ox, by = p.y + y + oy; if (bx < 0 || bx >= cols || by >= rows) return true;
    ...
    function merge(b, p) {
        const m = p.shape;
        for (let y = 0; y < m.length; y++) for (let x = 0; x < m[y].length; x++) if (m[y][x] && p.y + y >= 0) b[p.y + y][p.x + x] = p.color;
    }
    function clearLines() {
        let lines = 0;
        outer: for (let y = rows - 1; y >= 0; y--) {
            for (let x = 0; x < cols; x++) if (!board[y][x]) continue outer;
            board.splice(y, 1); board.unshift(Array(cols).fill(0)); lines++; y++;
        } if (lines > 0) score += lines * 100; document.getElementById('tetrisScore').textContent = score;
    }
    function spawn() {
        current = nextPiece || randPiece();
        nextPiece = randPiece();
        current.x = Math.floor((cols - current.shape[0].length) / 2);
        current.y = -1; if (collide(board, current)) running = false;
    }
    ...
    function draw() {
        ...
        if (current) {
            ...
            for (let y = 0; y < m.length; y++) for (let x = 0; x < m[y].length; x++) if (m[y][x]) drawCell(current.x + x, current.y + y, current.color);
        }
    }
    function drop() {
        if (!current) return;
        if (!collide(board, current, 0, 1)) current.y++;
            ...
            merge(board, current);
            clearLines();
            score += 10; document.getElementById('tetrisScore').textContent = score;
            spawn();
        }
        draw();
    }
    ...
    function move(dx) {
        if (current && !collide(board, current, dx, 0)) current.x += dx;
        draw();
    }
    function rotatePiece() {
        if (current) {
            const rotated = rotate(current.shape);
            const oldShape = current.shape; current.shape = rotated;
